...
Shared ESLint configuration for Lux projects, plus a set of custom lint rules.

This package was extracted from the [luxfi/exchange](https://github.com/luxfi/exchange) monorepo into a standalone package.

## Install

```bash
pnpm add -D @luxfi/eslint-config eslint@8.57.1
```

## Usage

Extend the appropriate config in your `.eslintrc`:

```json
{
  "extends": ["@luxfi/eslint-config/base"]
}
```
...
| Entry | Purpose |
|-------|---------|
| `base.js` | Base config for TypeScript projects (package `main`) |
| `native.js` | React Native config |
| `lib.js` | Library config |
| `load.js` | Loader helper |
| `biome-supported.js` | Subset of rules also supported by Biome |
| `restrictedImports.js` | Shared restricted-imports policy |

Custom rules live in `plugins/`, with shared helpers in `utils/`.
